        ...
        <div class="w-full">
            ...
            <div class=" flex flex-row">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr class=" text-center border-b  border-[#2c7fd2]">
                                    ...
                                    <td class=" w-1/3">
                                        <form action="alumnoClasesDelete.php" method="post" class=" flex justify-center items-center">
                                            <button class=" text-red-600 " value="<?= $idCurso ?>" name="eliminar">
                ...
                <div class="bg-white p-4 w-[40%] m-5 rounded-xl shadow-lg text-[#8FA5BD]">
                    ...
                    <hr>
                    <form action="alumnoClasesEdit.php" method="post">
                        <h2 class=" font-semibold py-3">Selecciona la(s) Clase(s)</h2>

                        <?php
                        $queryCursos = "SELECT * FROM cursos LEFT JOIN usuario_curso on cursos.id_curso=usuario_curso.id_curso AND usuario_curso.id_usuario = '$idAlumno' WHERE usuario_curso.id_usuario_curso IS NULL;";
                        $datosCursos = $mysqli->query($queryCursos);


                        while ($fila = $datosCursos->fetch_assoc()) {

                            $nombreCursoNA = $fila["nombre_curso"];
                            $queryTodosCursos = "SELECT * FROM cursos WHERE nombre_curso='$nombreCursoNA'";
                            $traer = $mysqli->query($queryTodosCursos);
                            $resultado = $traer->fetch_assoc();
                            $idCursoNA = $resultado["id_curso"];

                        ?>
                            <div class=" flex flex-row gap-3 items-center">
                                <input type="checkbox" name="cursos[]" id="curso<?= $idCursoNA ?>" value="<?= $idCursoNA ?>">
                                <label for="curso<?= $idCursoNA ?>"><?= $nombreCursoNA ?></label>
                            </div>
                        <?php
                        }
                        ?>
